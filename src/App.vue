		...
		<div class="container">
			<section>
				<div class="flex">
					<div class="max-w-xs">
						...
						<div
							v-if="similarTickers.length > 0"
							class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
							<span
								v-for="(similarTicker, i) in similarTickers"
								@click="addSimilar(similarTicker.Symbol)"
								:key="i"
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
								{{ similarTicker.Symbol }}
							</span>
						</div>
						<div v-if="isTickerInArr" class="text-sm text-red-600">
							Такой тикер уже добавлен
						</div>
						<div
							v-if="!isTickerExists && this.ticker !== ''"
							class="text-sm text-red-600">
							Такого тикера у нас нет
						</div>
					</div>
				</div>
				<button
			...
			<section v-if="sel" class="relative">
				...
				<div
					...
					<div
						v-for="(column, i) in normalizeGraph()"
						:key="i"
						:style="{ height: `${column}%` }"
...
export default {
	...
	data() {
		return {
			api_key:
				"22ccfa59d43e31e3c55a10e91a6020775f77738dc7d86de2116c228044243a5c",
	...
	methods: {
		...
		subscribeToUpdate(ticker) {
			ticker.intervalId = setInterval(async () => {
				let response = await fetch(
					`${this.api_url}/price?fsym=${ticker.name}&tsyms=USD&api_key=${this.api_key}`
				);
		...
		add() {
			...
			if (this.isTickerInArr || !this.isTickerExists) {
				return;
			}
		...
		select(tickerToSelect) {
			if (this.sel != tickerToSelect) {
		...
		async fetchAllTickers() {
			let response = await fetch(
				`${this.api_url}/all/coinlist?summary=true&filter=USD`
			);
			response = await response.json();
	...
	computed: {
		isTickerInArr() {
			if (this.tickers.length === 0 || this.ticker.length === 0) {
				return;
			}
		...
		isTickerExists() {
			...
			return this.allTickers.some(
				(t) =>
					t.Symbol.includes(trimmedTicker) ||
					t.FullName.toUpperCase().includes(trimmedTicker)
			);
		},
